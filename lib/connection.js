 ...
 * @param {String} organizationId - Salesforce organizationId 
 * @param {Callback.<UserInfo>} [callback] - Callback function
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.login = function (username, password, organizationId, callback) {
  if (this.oauth2) {
    return this.loginByOAuth2(username, password, organizationId, callback);
  } else {
    return this.loginBySoap(username, password, organizationId, callback);
  }
};
 ...
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.loginByOAuth2 = function (username, password, organizationId, callback) {
  var self = this;
  var logger = this._logger;
 ...
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.loginBySoap = function (username, password, organizationId, callback) {
  var self = this;
  var logger = this._logger;
  var body = [
    ...
    '<urn:organizationId>' + esc(organizationId) + '</urn:organizationId>',
    '</urn:LoginScopeHeader>',
    '</soapenv:Header>',
    '<soapenv:Body>',
    '<urn:login>',
    '<urn:username>' + esc(username) + '</urn:username>',
    '<urn:password>' + esc(password) + '</urn:password>',
    '</urn:login>',
    '</soapenv:Body>',
    '</soapenv:Envelope>'
  ].join('');
  ...
  return request({
    ...
    var serverUrl = m && m[1];
    m = response.body.match(/<sessionId>([^<]+)<\/sessionId>/);
    sessionId = response.body.match(/<sessionId>([^<]+)<\/sessionId>/);
    var sessionId = m && m[1];
    m = response.body.match(/<userId>([^<]+)<\/userId>/);
    var userId = m && m[1];
    m = response.body.match(/<organizationId>([^<]+)<\/organizationId>/);
...
Connection.prototype.logout = function (callback) {
  ...
  var body = [
    ...
    '<soapenv:Header>',
    '<urn:SessionHeader>',
    '<urn:sessionId>' + esc(this.accessToken) + '</urn:sessionId>',
    '</urn:SessionHeader>',
    '</soapenv:Header>',
    '<soapenv:Body>',
    '<urn:logout/>',
